...
#note: yes, the first set is downloaded in parallel and the second is downloaded in sequence. I don't like waiting, but if I do them all in parallel, then the script apparenly can finish before curl and the terminal gets messy. So yes, this is an ugly hack of a fix
